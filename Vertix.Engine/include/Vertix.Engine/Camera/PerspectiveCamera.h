...
namespace Vertix::Engine {
    ...
    public:
        ...
        VERTIX_ENGINE_API void Move(
            ...
            bool                                relative  = true,
            bool                                allowRoll = false) noexcept override;

        VERTIX_ENGINE_API void Rotate(
            const DirectX::SimpleMath::Vector3 &angles,
            bool                                allowRoll = false) noexcept override;

        void GetViewMatrix(DirectX::SimpleMath::Matrix &view) const noexcept { view = DirectX::SimpleMath::Matrix::CreateLookAt(position, position + forward, up); }
        void GetProjectionMatrix(DirectX::SimpleMath::Matrix &projection) const noexcept { projection = projectionMatrix; }
        ...
        [[nodiscard]] float GetAspect() const noexcept { return aspect; }
        [[nodiscard]] float GetFieldOfView() const noexcept { return fov; }
