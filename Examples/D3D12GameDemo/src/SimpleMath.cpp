...
RECT Viewport::ComputeDisplayArea(DXGI_SCALING scaling, UINT backBufferWidth, UINT backBufferHeight, int outputWidth, int outputHeight) noexcept
{
    ...
    switch (static_cast<int>(scaling))
    {
    ...
    case 2 /*DXGI_SCALING_ASPECT_RATIO_STRETCH*/:
        ...
        // Note: This scaling option is not supported for legacy Win32 windows swap chains
        {
            ...
            const float aspectRatio = static_cast<float>(backBufferWidth) / static_cast<float>(backBufferHeight);

            // Horizontal fill
            float scaledWidth = static_cast<float>(outputWidth);
            float scaledHeight = static_cast<float>(outputWidth) / aspectRatio;
            if (scaledHeight >= static_cast<float>(outputHeight))
            {
                // Do vertical fill
                scaledWidth = static_cast<float>(outputHeight) * aspectRatio;
                scaledHeight = static_cast<float>(outputHeight);
            }
...
RECT Viewport::ComputeTitleSafeArea(const UINT backBufferWidth, const UINT backBufferHeight) noexcept
{
    const float safew = (static_cast<float>(backBufferWidth) + 19.f) / 20.f;
    const float safeh = (static_cast<float>(backBufferHeight) + 19.f) / 20.f;

    RECT rct;
    rct.left = static_cast<LONG>(safew);
    rct.top = static_cast<LONG>(safeh);
    rct.right = static_cast<LONG>(static_cast<float>(backBufferWidth) - safew + 0.5f);
    rct.bottom = static_cast<LONG>(static_cast<float>(backBufferHeight) - safeh + 0.5f);
