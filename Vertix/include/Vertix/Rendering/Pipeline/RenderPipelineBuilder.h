...
namespace Vertix {
    ...
    public:
        VERTIX_API explicit RenderPipelineBuilder(GraphicsDevice* graphicsDevice);
        ...
        public:
            template <RenderPassType TRenderPass, typename TDeclareFunc, typename... CtorArgs> requires std::invocable<TDeclareFunc, PassDeclarationBuilder<TRenderPass>&>
            void Add(
                TDeclareFunc &&declareFunc,
                CtorArgs &&... ctorArgs) noexcept {
                PassDeclarationBuilder<TRenderPass> builder;
                builder.Construct(ctorArgs...);
                builder.registerView = [&](const std::string &                resourceName,
