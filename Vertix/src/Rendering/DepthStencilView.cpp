...
Vertix::DepthStencilView::DepthStencilView(const GraphicsDevice *graphicsDevice,
                                           const D3D12_RESOURCE_DESC &dsvResourceDesc,
                                           const D3D12_CPU_DESCRIPTOR_HANDLE &descriptorHandle,
                                           const D3D12_DEPTH_STENCIL_VIEW_DESC* dsvDesc,
                                           const D3D12_CLEAR_VALUE &clearValue)
    : d3d12Device(graphicsDevice->GetD3D12Device()), dsvHandle(descriptorHandle), dsvResourceDesc(dsvResourceDesc), clearValue(clearValue) {
...
CD3DX12_RESOURCE_BARRIER Vertix::DepthStencilView::CreateTransitionBarrier(const D3D12_RESOURCE_STATES before,
